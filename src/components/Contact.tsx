...
import { useSocialLinks, useContactInfo, useBusinessHours } from "@/hooks/use-site-data";
...
const Contact = () => {
  ...
  const { data: contact, isLoading: loadingContact } = useContactInfo();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {loadingLinks
                ...
                : links?.map((link: any) => (
                    <a
                      ...
                      className={`${colorMap[link.icon] || "bg-foreground"} text-primary-foreground flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold hover:opacity-85 transition-opacity`}
                    >
                      {iconMap[link.icon] || null}
                      {link.name === "Email" ? contact?.email || link.name : link.name}
                    </a>
                  ))}
            </div>
          </div>
          ...
          <div className="bg-background rounded-xl p-6 border border-border shadow-sm">
            ...
            {loadingContact ? (
              ...
              <>
                <p className="text-muted-foreground text-sm mb-4">{contact?.address}</p>
                <a
          ...
          <div className="bg-background rounded-xl p-6 border border-border shadow-sm">
            ...
            <div className="space-y-3 text-sm">
              {loadingHours
                ...
                : hours?.map((h: any) => (
                    <div key={h.id}>
                      <p className="font-medium text-foreground">{h.day_label}</p>
                      {h.hours?.map((line: string, i: number) => (
